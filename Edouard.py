...
class CrosswordGrid:
    def __init__(self, size, list):
        self.table = np.full((size, size), None, dtype=str)
        self.words = []
        self.letters = []
        self.words_list=list

    def check_word_fits_in_grid_and_place_it(self, word, position):
        # Vérifie si le mot peut être placé à la position spécifiée dans la grille
        if word.is_horizontal:
            return self.check_horizontal_placement(word, position)
        else:
            return self.check_vertical_placement(word, position)

    def check_horizontal_placement(self, word, position):
        # Vérifie si le mot peut être placé horizontalement à la position spécifiée
        for i, letter in enumerate(word.name):
            if (
                position[0] + i >= self.table.shape[0] or
                self.table[position[0] + i, position[1]] is not None and
                self.table[position[0] + i, position[1]] != letter
            ):
                return False
        return True

    def check_vertical_placement(self, word, position):
        # Vérifie si le mot peut être placé verticalement à la position spécifiée
        for i, letter in enumerate(word.name):
            if (
                position[1] + i >= self.table.shape[1] or
                self.table[position[0], position[1] + i] is not None and
                self.table[position[0], position[1] + i] != letter
            ):
                return False
        return True

    def place_word_in_grid(self, word, position):
        # Place le mot dans la grille à la position spécifiée
        if word.is_horizontal:
            for i, letter in enumerate(word.name):
                self.table[position[0], position[1] + i] = letter
        else:
        ...
        # Met à jour les attributs du mot
        word.first_letter_position = position
...
# Liste complète de 30 mots (exemple)
all_words_list = [
    "python", "algorithmes", "programmation", "intelligence", "artificielle",
    "apprentissage", "données", "analyse", "réseau", "web",
    "développement", "langage", "logiciel", "robotique", "système",
    "structure", "informatique", "interface", "utilisateur", "application",
    "cryptographie", "sécurité", "cloud", "mobile", "navigateur",
    "serveur", "prototype", "optimisation", "conception", "interopérabilité"
]

# Choisissez aléatoirement 10 mots parmi la liste complète
all_words_list = [
    "python", "algorithmes", "programmation", "intelligence", "artificielle",
    "apprentissage", "données", "analyse", "réseau", "web",
    "développement", "langage", "logiciel", "robotique", "système",
...
# Choisissez aléatoirement 10 mots parmi la liste complète
selected_words_list = [Word(word, "ABC") for word in random.sample(all_words_list, 10)]

# Créez une grille de mots croisés avec les 10 mots sélectionnés
crossword_grid = CrosswordGrid(15, selected_words_list)
crossword_grid.generate(selected_words_list)

# Affichez la grille
print(crossword_grid.table)
